...
// The symbol table is a table of pointers to symbol (which can be of various
// types)
...
symbol_table::symbol_table() {
...

  {
    ...
    sym_index read_sym =
        enter_function(dummy_pos, pool_install(capitalize("read")));
  ...
  }
  {
    ...
    sym_index write_sym =
        enter_procedure(dummy_pos, pool_install(capitalize("write")));
    sym_index int_arg = enter_parameter(
        dummy_pos, pool_install(capitalize("int-arg")), integer_type);
  ...
  // a real argument.
  sym_index trunc_sym =
      enter_function(dummy_pos, pool_install(capitalize("trunc")));
  ...
  sym_index real_arg = enter_parameter(
      dummy_pos, pool_install(capitalize("real-arg")), real_type);
  ...
  case 1:
    ...
    for (int i = 0; i < sym_pos + 1; i++) {
      ...
      case SYM_PROC: {
        ...
        cout << "SYM_PROC" << setw(6) << "lbl = " << setw(3) << proc->label_nr
             << setw(9) << "ar_size = " << setw(3) << proc->ar_size;
        break;
      }
      case SYM_FUNC: {
        function_symbol *func = tmp->get_function_symbol();
        cout << "SYM_FUNC" << setw(6) << "lbl = " << setw(3) << func->label_nr
             << setw(9) << "ar_size = " << setw(3) << func->ar_size;
        break;
      }
      case SYM_ARRAY: {
      ...
      case SYM_CONST: {
        ...
        if (con->type == integer_type)
          cout << "SYM_CONST" << setw(7) << "value = " << con->const_value.ival;
        else if (con->type == real_type)
          cout << "SYM_CONST" << setw(7) << "value = " << con->const_value.rval;
        else
...
/* Return a sym_index to the sought symbol (or NULL_SYM if none was found),
   given a string_pool index. Starts searching in the current block level and
   follows hash links outwards. */
sym_index symbol_table::lookup_symbol(const pool_index pool_p) {
  /* Your code here */
  hash_index hash_value = hash(pool_p);
  sym_index sym_idx = hash_table[hash_value];
  // follow the hash link backwards just means search from innermost scope to
  // outermost scope due to the order of installation
  while (sym_idx != NULL_SYM) {
    symbol *tmp = sym_table[sym_idx];
    if (pool_p == tmp->id || pool_compare(pool_p, tmp->id)) {
      return sym_idx;
    }
    sym_idx = tmp->hash_link;
  }
  return NULL_SYM;
}
...
/* Install a symbol in the symbol table or return a sym_index to it if it was
   ...
   The parameter 'tag' can have one of eight different types, see the file
   symtabb.hh for the type definition of sym_type. Remember that the attribute
   'tag' and 'id' will be set when creating a new symbol inside the symbol
   constructor (take a look at symbol.cc).*/

sym_index symbol_table::install_symbol(const pool_index pool_p,
                                       const sym_type tag) {
  /* Your code here */
  // cout << "Installing symbol: " << tag << "-" << pool_lookup(pool_p) << " in
  // current level: " << current_level << endl; step 1 check if the symbol
  // already exists in same scope
  sym_index existing_sym_index = lookup_symbol(pool_p);
  if (existing_sym_index != NULL_SYM &&
      sym_table[existing_sym_index]->level == current_level) {
    // symbol already exists in same scope, return the index
    // cout << "Symbol already exists in same scope, returning index: " <<
    // existing_sym_index << endl;
    return existing_sym_index;
  }
  // step 2 find the bucket in hash table
  hash_index hash_value = hash(pool_p);
  sym_index old_sym_idx =
      hash_table[hash_value]; // the symbol at the top of the bucket
  // cout << "old_sym_idx = " << old_sym_idx << endl;
  // step 3 create the new symbol and mount it to the top of the bucket, set the
  // hash link and backlink
  symbol *new_symbol;
  switch (tag) {
  case SYM_ARRAY:
  ...
  case SYM_NAMETYPE:
    ...
    break;
  case SYM_UNDEF:
    new_symbol = new symbol(pool_p);
    break;
  default:
    fatal("Invalid symbol type");
    return NULL_SYM;
  }
  sym_index new_sym_idx = ++sym_pos;
  // cout << "new_sym_idx = " << new_sym_idx << endl;
  sym_table[new_sym_idx] = new_symbol;
  new_symbol->hash_link = old_sym_idx;
  ...
  hash_table[hash_value] =
      new_sym_idx; // mount the new symbol to the top of the bucket

  // step 4 set other attributes of the new symbol
  new_symbol->level = current_level;

  // cout << "new_symbol: " << pool_lookup(pool_p) << " level: " <<
  // new_symbol->level << "idx: " << new_sym_idx << endl;

  return new_sym_idx; // Return index to the symbol we just created.
}
...
sym_index symbol_table::enter_constant(position_information *pos,
                                       ...
                                       const sym_index type, const long ival) {
