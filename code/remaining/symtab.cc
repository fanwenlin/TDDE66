...
sym_index symbol_table::close_scope()
{
    ...
    for (int i = block_table[current_level] + 1; i <= sym_pos; i++)
    {
        ...
        // cout << "Deleting symbol: " << tmp << endl;
        // ? should we delete the symbol here?
        // delete tmp;
    }

    // ? shouldn't we?
    // sym_pos = block_table[current_level];
...
sym_index symbol_table::lookup_symbol(const pool_index pool_p)
{
    ...
    // follow the hash link backwards just means search from innermost scope to outermost scope
    // due to the order of installation
    while (sym_idx != NULL_SYM)
    {
        symbol *tmp = sym_table[sym_idx];
        if (pool_p == tmp->id ||
            pool_compare(pool_p, tmp->id))
        {
            return sym_idx;
        }
...
{
    /* Your code here */
    // cout << "Installing symbol: " << tag << "-" << pool_lookup(pool_p) << " in current level: " << current_level << endl;
    // step 1 check if the symbol already exists in same scope
    sym_index existing_sym_index = lookup_symbol(pool_p);
    if (existing_sym_index != NULL_SYM && sym_table[existing_sym_index]->level == current_level)
    {
        // symbol already exists in same scope, return the index
        // cout << "Symbol already exists in same scope, returning index: " << existing_sym_index << endl;
        return existing_sym_index;
    }
    // step 2 find the bucket in hash table
    hash_index hash_value = hash(pool_p);
    sym_index old_sym_idx = hash_table[hash_value]; // the symbol at the top of the bucket
    // cout << "old_sym_idx = " << old_sym_idx << endl;
    // step 3 create the new symbol and mount it to the top of the bucket, set the hash link and backlink
    symbol *new_symbol;
    switch (tag)
    {
    case SYM_ARRAY:
    ...
    // cout << "new_symbol: " << pool_lookup(pool_p) << " level: " << new_symbol->level << "idx: " << new_sym_idx << endl;
